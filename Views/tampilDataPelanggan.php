        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                   <a href="../Insert/insertDataPelanggan.php">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-x text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-center <?php if($_COOKIE['levelUser'] != "admin" ){ echo "hidden";}?>">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                         <?php 
                        ...
                        <tr class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= $data['idPelanggan'] ?></th>
                            <td class="px-4 py-3 "><?= $data['namaLengkap'] ?></td>
                            <td class="px-4 py-3 "><?= $data['alamat'] ?></td>
                            <td class="px-4 py-3 "><?= $data['telp'] ?></td>
                            <td class="px-4 py-3 "><?= $data['usia'] ?></td>
                                <td class="px-4 py-3 ">
                                <img class=" object-contain w-24 h-24" src="img/resep/<?=$data['buktiFotoResep']?>" alt="">
                                </td>
                            <td class="px-3 py-3 flex  gap-2 items-center justify-center <?php if($_COOKIE['levelUser'] != "admin" ){ echo "hidden";}?>">
                            <button type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5  dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                <a href="dashboard.php?page=updateDataPelanggan&id=<?=$data['idPelanggan']?>">Edit</a>
                            </button>
                            <button type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5  dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                             <a href="dashboard.php?page=deleteDataPelanggan&id=<?=$data['idPelanggan'] ?>">Delete</a>
                        </button>
                        </td>
                        </tr>
